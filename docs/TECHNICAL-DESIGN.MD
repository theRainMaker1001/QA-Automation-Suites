...
* **E2E (20+ Tests):** High-fidelity user simulation focusing on critical paths.
* **Integration (84 Tests):** Fast API contract validation and Zod-based schema integrity.
...
To ensure absolute rigor for this financial system, we 3-Value BVA test cases covering the **Invalid Partition (-2, -1)**, the **Boundary (0)**, and the **Valid Partition (+1, +2)**.
...
stateDiagram-v2
    ...
    LoggedIn --> LockedOut : 3x Bad Attempts
...
The pipeline utilizes **GitHub Actions** with strict quality gates. This multi-lane approach provides rapid feedback for developers while maintaining full regression confidence.
...
graph LR
    ...
    direction TB
    CI --> Smoke[Running @smoke]
    Smoke -->|Pass| Critical[Running @critical]
    end
    
    subgraph "Scheduled Lanes"
    direction TB
    Nightly[🕐 Nightly] --> Regression[Running @regression]
    Nightly --> A11y[Running @a11y]
    Hourly[⏰ Hourly] --> Heartbeat[Running @heartbeat]
    end
...
* **Cent-to-Dollar Logic:** All internal arithmetic is performed in integer space (cents) to avoid precision loss.
...
npm run test:smoke      # 30s Connectivity check
